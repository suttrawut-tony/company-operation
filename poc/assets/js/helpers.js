...
function statusBadge(s) {
  const cls = s === 'overdue' ? 'danger' : s === 'revised' ? 'purple' : s.includes('pending') ? 'warning' : ['approved','completed','paid','checked_in'].includes(s) ? 'success' : s === 'draft' ? 'gray' : ['sent_to_sap','received','in_progress'].includes(s) ? 'primary' : s === 'rejected' ? 'danger' : 'gray';
